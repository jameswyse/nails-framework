...
App.prototype.configure = function configure(opts, callback) {
  ...
  function configureCache(next) {
    ...
    if(opts.cache && utils.is.string(opts.cache.engine)) {
      Engine = this.requireFn(opts.cache.engine);
    }
    else {
      Engine = opts.cache.engine || require('catbox-memory');
    }

    delete opts.cache.engine;

    opts.cache = {
      engine: new Engine(opts.cache)
    };

    opts.cache.engine.start(function(err) {
...
App.prototype.start = function startApp(callback) {
  ...
  // Wait 100ms if app is still configuring
  if(!this._configured && this._configuring) {
