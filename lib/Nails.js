...
Nails.prototype.init = function() {
  ...
  //
  // Compile a list of required plugins
  //
  var plugins = {
    'hapi-promise': {},
    'hapi-named-routes': {},
...
Nails.prototype.plugin = function usePlugin(pattern, options) {
  assert(is.string(pattern), 'Missing glob pattern(s)');
  options = options || {};
  ...
  // var stat = utils.fs.statSync(pattern);

  // if(stat.isDirectory()) {
  //   // check if index.js exists - pattern = pattern + '/index.js';
  //   // otherwise pattern = pattern + '/**/*.js';
  // }
  // else if(stat.isFile()) {
