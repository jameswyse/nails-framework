...
var Registry   = require('nails-registry');
...
var Nails = function(opts) {
  ...
  this.name        = opts.name        || this.pkg.name        || 'app';
  this.description = opts.description || this.pkg.description || 'app';
  this.version     = opts.version     || this.pkg.version     || '0.0.0';


  // Expose Hapi and some common methods
  this.hapi    = hapi;
  this.utils   = utils;
  this.logger  = intel;
...
Nails.prototype.model    = model;
Nails.prototype.log      = new utils.DummyLogger();
Nails.prototype.task     = new Agenda();
Nails.prototype.service  = new Registry('Service');
...
Nails.prototype.init = function() {
  ...
  if(logConfig && logConfig.handlers) {
    ...
    this.log = this.logger.getLogger(this.name || 'app');
  ...
  //
  // Init: App Hapi Plugins
  //
  var appPlugins = opt('plugins');

  if(!is.array.empty(appPlugins)) {
    appPlugins.forEach(function(pattern) {
      self.plugin(pattern, {});
    });
  }
  ...
  if(tasks && mongoURL) {
    ...
    this.task
      ...
      .on('start', function(job) {
        taskLogger.info('Starting: %s', cyan(job.attrs.name));
      })
      .on('success', function(job) {
        taskLogger.info('Complete: %s', cyan(job.attrs.name));
      })
      .on('fail', function(err, job) {
        taskLogger.warn('Failed: %s', red(job.attrs.name));
...
Nails.prototype.server = function createServer(host, port, options) {
  ...
  server.on('log', function(event, tags) {
    ...
    self.log[level](util.format(event.data));
  });

  // Log Hapi request errors
  ...
  this.pack.start(function() {
    ...
    if(self.task) {
      ...
      self.task.purge(function(err, num) {
        if(err) tLog.error(err);
        if(num) tLog.info('Purged %i old jobs', cyan(num));

        // Start scheduler after 0-5 seconds
        // Helps to prevent multiple servers checking at the same time
        setTimeout(function() {
          self.task.start();
